...
function BootScreen({ onDone }) {
  ...
  return (
    ...
    >
      ...
      <div
        className="text-xs tracking-[.2em]"
        style={{ color: "rgba(0,255,65,.5)" }}
      >
        CYBER OPERATIONS CENTER v4.2.1
      </div>
      <div
        className="w-[min(520px,90vw)] h-40 overflow-hidden text-xs"
        style={{ color: "#00cc33" }}
      >
        {lines.map((l, i) => (
      ...
      <div
        className="w-[min(520px,90vw)] h-[6px] rounded-sm overflow-hidden border"
        style={{ background: "rgba(0,255,65,.1)", borderColor: "#00cc33" }}
      >
        <div
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-3 flex-wrap gap-2">
        ...
        <div className="flex items-center gap-3">
          <div
            className="hidden sm:block text-xs px-2 py-1 border"
            style={{
              color: "#ff003c",
              borderColor: "#ff003c",
      ...
      {/* Nav links */}
      <div
        className={`${menuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row md:items-center gap-0 md:gap-1 px-5 pb-2 md:pb-0`}
...
function DonutChart() {
  ...
  return (
    <div className="flex items-center gap-4 mt-1">
      <svg
        viewBox="0 0 100 100"
        style={{ width: 115, height: 115, flexShrink: 0 }}
      >
        {slices.map((s) => (
          <path
            key={s.l}
...
function Gauge({ value }) {
  ...
  useEffect(() => {
    ...
    ctx.beginPath();
    ctx.moveTo(cx, cy);
    ctx.lineTo(cx + 65 * Math.cos(na), cy + 65 * Math.sin(na));
...
const LOG_TMPL = [
  {
    cl: "text-red-400",
    m: () => `[BLOCK] SQL injection from ${rip()} → /api/auth`,
  },
  {
    cl: "text-red-400",
    m: () => `[ALERT] Brute-force: ${rip()} (${ri(50, 500)} attempts)`,
  },
  {
    cl: "text-yellow-400",
    m: () =>
      `[WARN]  Port scan: ${rip()} range ${ri(1, 30000)}-${ri(30001, 65535)}`,
  },
  {
    cl: "text-yellow-400",
    m: () => `[WARN]  Traffic spike: ${ri(200, 999)}Mbps on eth0`,
  },
  {
    cl: "",
    m: () => `[INFO]  Rule #${ri(1000, 9999)} triggered → DROP ${rip()}`,
  },
  { cl: "", m: () => `[INFO]  DNS anomaly: ${rip()} querying C2` },
  {
    cl: "text-cyan-400",
    m: () => `[OK]    Packets cleared: ${ri(100, 999)}/s`,
  },
  {
    cl: "text-cyan-400",
    m: () => `[OK]    Sig DB updated: ${ri(10000, 99999)} rules`,
  },
  { cl: "text-red-400", m: () => `[ALERT] RCE attempt: ${rip()} → /cgi-bin/` },
  {
    cl: "text-yellow-400",
    m: () =>
      `[WARN]  Geo-block: ${rip()} [${["CN", "RU", "KP", "IR"][ri(0, 4)]}]`,
  },
];
// function LiveLog() {
...
//       style={{ height: 120, color: "rgba(0,255,65,.7)", lineHeight: 1.75 }}
//     >
//       {logs.map((l) => (
//         <div
//           key={l.id}
//           className={`whitespace-nowrap overflow-hidden ${l.cls}`}
//         >
//           {l.text}
//         </div>
//       ))}
...
//               [{r.co}]
//             </td>
//             <td className="py-[5px] text-right" style={{ color: r.c }}>
//               {r.h.toLocaleString()}
//             </td>
//           </tr>
//         ))}
      ...
      <tbody>
        {rows.map((r, idx) => (
          ...
          >
            ...
            <td className="py-[5px] text-right" style={{ color: "#ff003c" }}>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 px-5 py-3">
        <div className="flex items-center gap-4">
          ...
          <span style={{ color: "rgba(0,255,65,.45)" }}>
            SESSION UPTIME:{" "}
            <span style={{ color: "#00f5ff" }}>{fmt(uptime)}</span>
          </span>
          <span style={{ color: "rgba(0,255,65,.35)" }}>|</span>
          <span style={{ color: "rgba(0,255,65,.45)" }}>
...
function Dashboard() {
  ...
  const [logs, setLogs] = useState([]);
  const [topAttackIPs, setTopAttackIPs] = useState([]);
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      try {
        ...
        setStats(data.stats);
        setRes(data.resource_usage);
        setGaugeVal(data.risk_score);
        setTIn(data.traffic.inbound || []);
        setTOut(data.traffic.outbound || []);
        setLogs(data.event_logs || []);
        setTopAttackIPs(data.top_attack_sources || []);
        setProtocolData(data.protocols || {});
        setAttackData(data.attacks || {});
        setLivePackets(data.live_packets || []);
      } catch (err) {
  ...
  return (
    <div className="p-3 font-mono-hack" style={{ color: "#00ff41" }}>
      ...
      <div
        ...
        style={{ gridTemplateColumns: "240px 1fr 260px" }}
      >
        {/* ─── LEFT SIDEBAR ─── */}
          ...
          <Panel title="Active Threats">
            {[
              ...
              return (
                <div
                  key={n}
                  className="flex justify-between items-center py-[6px] text-[.68rem]"
                  style={{ borderBottom: "1px solid rgba(0,255,65,.07)" }}
                >
        ...
        <div className="flex flex-col gap-3">
          ...
          <Panel title="Live Packet Predictions">
